...
def save_results(input_img, gt_data, density_map, output_dir, fname='results.png'):
    ...
    #cv2.imwrite(os.path.join('.', output_dir, fname).replace('.h5', 'refine.bmp').replace('.jpg', 'refine.bmp'),
    #            density_map)

    result_img = np.hstack((gt_data, density_map))
    #print(os.path.join('.', output_dir, fname).replace('.h5', 'fuse.jpg').replace('.jpg', '.fuse.jpg'))
    cv2.imwrite(os.path.join('.', output_dir, fname).replace('.h5', 'fuse.jpg').replace('.jpg', '.fuse.jpg'),
                result_img)
